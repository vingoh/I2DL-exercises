...
class DataLoader:
    ...
    def __iter__(self):
        ...
        if not self.drop_last and (len(self.dataset) % self.batch_size):
            keep_last = True
        else: keep_last = False
        num_batches = len(self.dataset) / self.batch_size

        if self.shuffle:
            index = iter(np.random.permutation(len(self.dataset)))
        else:
            index = iter(range(len(self.dataset)))

        for i in index:
            batch = np.append(batch, self.dataset[i]["data"])
            count += 1

            if count == len(self.dataset) and keep_last:
                batch_dict = {"value": batch}
                yield batch_dict
