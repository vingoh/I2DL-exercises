...
class RescaleTransform:
    ...
    def __call__(self, images):
        ...
        for i in range(images.shape[0]):
            images[i] = images[i]/(self._data_max - self._data_min)
...
def compute_image_mean_and_std(images):
    ...
    m = images.mean(axis=(0, 1, 2))
    s = images.std(axis=(0, 1, 2))
        ...
        for j in range(images.shape[3]):
            for k in range(images.shape[1]):
                for l in range(images.shape[2]):
                    sum = sum + images[i,k,l,j]
        mean[j] = sum/(images.shape[1]*images.shape[2])
    """
