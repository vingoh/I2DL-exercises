...
class Classifier(Network):
    ...
    def initialize_weights(self, weights=None):
        ...
        if weights is not None:
            ...
            "返回一个元素均符合标准正态分布N(0,1)的num_features+1 * 1的数组"
    ...
    def forward(self, X):
        ...
        # save the samples for the backward pass，加了一列1在X最后
        self.cache = X
        # output variable
        y = None
    ...
    def backward(self, y):
        ...
        sig_XW = self.sigmoid(np.dot(self.cache, self.W))
        dW = self.cache * (sig_XW * (1 - sig_XW))
